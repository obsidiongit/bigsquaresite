...
- **Native scroll is locked.** No Lenis, no scroll hijack, no virtual scroller (every analysis reached this conclusion; SEO, accessibility, LCP). Reveals fire once (`whileInView`, `once: true`); the pxpush pattern of scrub-reversible body text is explicitly not taken. The one sanctioned assist is the idle-settle scroll checkpoint (7.4): it acts only after scrolling has stopped, never touches input in flight, and any input cancels it instantly.
