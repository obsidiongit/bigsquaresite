...
EXTRACTED 2026-08-26 (`components/shared/Field.tsx`, the /schedule/ application form, Lane 1): the shared primitive carries this whole contract, with a visible mono-uppercase label by default (`hideLabel` for compositions with no room). NewsletterForm still holds a local copy of the anatomy and migrates to `Field` in its next homepage session (tracked in tasks.md; its file is homepage-owned).
...
### 6.15 Interior breadcrumb row (2026-08-25)

The wayfinding open on interior pages (first shipped on the T2 service template). A mono meta row at the top of the page hero: `SERVICES / ORGANIC MARKETING` style, 13px mono uppercase in `--sec-mid`, each part a real link brightening to `--sec-ink` on hover, sitting under a drawn-in hairline (`SeparatorIn`). Semantically a `<nav aria-label="Breadcrumb">`. The page's BreadcrumbList JSON-LD mirrors the same trail with real page URLs only (Home stays JSON-LD-only; a group head that is an anchor stays visible-only). This is the interior counterpart of the hero's grandfathered film meta: measured detail at component scale, not page chrome (4.3).

### 6.16 Media slot: the interior asset system (2026-08-26)

Brad's round-1 gate note on the interior flagships ("no area for any assets... even placeholders for now, and I can drop the assets in at the end"). Every media moment on interior pages renders through the shared `MediaSlot`, keyed into the ASSET DROP-IN MAP (`lib/asset-files.ts`):

- **Unmapped id**: the DESIGNED placeholder: surf ground at `--radius-media`, 1px `--sec-line`, a ghost brand square, a mono ASSET chip, and the slot's `note` (what shot Brad should drop) in mono small. aria-hidden; honest, composed, and obviously temporary (the 6.12 honesty-gate move at page-content scale). Site chrome still never ships placeholders (6.8); page content under buildout does, designed.
- **Mapped id**: `next/image` with the slot's pre-written alt text; the chip and note drop automatically. Filling a slot is one file in `public/media/` plus one map row: never a layout change.
- `href` makes the whole framed object an internal link (image internal-linking, Brad's SEO ask) and gives a real image the 6.4 hover scale; never set it inside a parent link. Registration marks on lg+ (`marks={false}` in tight compositions). `aspect` takes a CSS ratio; `aspectClassName` overrides it for responsive ratios.
- Every slot is DEFINED in its page's content module (id, alt, note) and LISTED in `project-guidelines/asset-manifest.md` the session it ships; that manifest is Brad's end-of-build shopping list. Shared assets reuse one id across pages (the `industries-<slug>-card` images).
...
- **2026-08-26 (T2 flagship round 2: asset slots, image links, the variation kit)**: Brad's gate review of round 1 ("good, not great... a seven or eight out of 10... just a bit boring"; no asset slots, too template-repeatable). The fixes are system-level, new 6.16: every interior media moment renders through the shared `MediaSlot` wired to `lib/asset-files.ts` (designed placeholder with a mono ASSET chip + drop note until Brad maps a file; filling a slot is one file + one map row, never a layout change; `project-guidelines/asset-manifest.md` is his end-of-build shopping list), and images LINK INWARD wherever they have a natural target (work band to /results/, industry cards to their industry pages) with the 6.4 hover scale as the affordance. The T2 page gained: hero variant A media-right (square framed object + the page's token-native UI FRAGMENT overlapping its corner at xl: skeleton bars, no numbers, per-page composition; SEO's is "Rankings by location" rows with rising arrows), the wide work band between process and spine, industry links as linked IMAGE cards (a different texture from the rule-link rows beside them), and the spine circle on one heading phrase (T2 now spends 3 of 3: H1 underline, spine circle, CTA bracket; NOTE for gate arbitration alongside Lane 1's flag: Lane 3 recorded circle=T3 H1 ink, this circle is a spine-phrase mark, Brad calls it). The T2 brief (v2.1) adds the VARIATION KIT so 15 stamps do not read as one template: fixed system, per-page picks on hero variant (A media-right / B statement-wide / C fragment-led), fragment, band placement, ink placement, counts. CONCURRENT-LANE MERGE recorded: Lane 2 and Lane 3 both built a `MediaSlot` the same day (same Brad feedback, both panes); the surviving component keeps Lane 3's API and self-documenting placeholder (id, note, alt, CSS `aspect`) extended with Lane 2's `href` link wrapping, `marks` toggle, and responsive `aspectClassName`; both lanes' call sites verified against it.
- **2026-08-26 (T1 conversion flagship round 2: the Field primitive + the VSL object)**: Brad's round-1 review of /schedule/ ("really bland, really weak... 6 out of 10"; the calendar direction retired: booking is now a short in-page application form fed to the GHL API, decisions.md amendment). Two system changes fall out of the rebuild. (1) 6.13's `Field` primitive is EXTRACTED (`components/shared/Field.tsx`): the shared field with the visible mono-uppercase label as the default (the measured instrument voice on forms); NewsletterForm migrates in its next homepage session. (2) The VSL PLAYER pattern (section-local in `components/sections/schedule/VslPlayer.tsx`, promote here if a second page needs it): a click-to-play-WITH-SOUND framed film object, distinct from FramedMediaPanel's muted ambient loop; brand-square play button, native controls once playing, never autoplay, poster + inert play square + visible placeholder chip while the film asset is null (the 6.12 honesty-gate pattern applied to media). Also first conversion-page mount of `SquareField` (converging with Lane 3's interior mount, same review caveat) and a LOCATIONS PICKER form control (native radios styled as segment pills + a brand-square fill meter; section-local until a second form wants it). T1 ink note for Brad: /schedule/ runs H1 underline + a closing circle while Lane 3 recorded per-template H1-ink ownership (underline=T2, circle=T3); arbitrate at gate review.
- **2026-08-25 (T3 industry template flagship + the FAQ copy was never server-rendered)**: Lane 3 session (interior-buildout-plan.md), run in parallel with Lane 2's: `_industry-page-template.md` rewritten as the T3 design brief (grounded in two new captures: Youtech's own /franchises/ page and Ignite's franchise-marketing-company page, ANALYSIS.md files alongside) and the flagship `/industries/franchise/` built against it (IndustryPage layout + `lib/industry-pages/` registry, the same pages-exist-when-registered gate as T2). Consumed Lane 2's fresh shared primitives read-only per the lane rules; the one shared-file change is a FIX to `Faq.tsx`: Radix unmounts closed accordion answers, so NO FAQ answer copy was in the served HTML (probed by curl: the text existed only inside the FAQPage JSON-LD; a word-count claim that included it was counting the script). STYLE_GUIDE 10 requires copy server-side, so AccordionContent now `forceMount`s, and because Radix drops its `hidden` attribute after hydration under forceMount, a `[[data-state=closed]_&]:hidden` variant does the hiding (probed: without it every answer rendered open). This applies to /services/seo/ automatically. Template rules the build set, recorded in the T3 brief: the hero carries a quiet mono sub-market index in the small-right column, and the page's BREADTH BAND must never repeat it (round 1 rendered the same four chips twice; the franchise page runs a SECTORS variant whose chips become links to sibling industry pages); a T3 page carries a METHOD SPINE (3 to 4 H2 + paragraph blocks in the 65ch measure) as the word-count backbone, added when the composed sections measured ~1,080 words against the 1,500 contract (final: 1,534 server-rendered); the process moment is the 6.4 process card with DAY-RANGE chips composed as a lane-local caller variant (`PhaseCard`) per ProcessCard's own contract, phase ranges on screen and the invented milestone day numbers kept off; and the `--text-statement` slot is spent on the plan's payoff line closing the body. First interior mount of `SquareField` (ambient parity with the homepage; z-10 content wrappers per the layering contract): Brad may kill it sitewide-interior in review. T3 ink: circle on the industry word (T2 owns the underline), CTA bracket, 1 spare. Verified at 375/768/1280/1536 + reduced motion (0 running animations), no overflow, FAQ keyboard one-at-a-time, BreadcrumbList + FAQPage + Organization JSON-LD, 6 service links + hub + conversion links live.
- **2026-08-25 (T2 service template flagship: the first interior page, three shared closers)**: Lane 2 session 1 (interior-buildout-plan.md): `_service-page-template.md` rewritten as the T2 design brief under 4.5's open layout, and the flagship `/services/seo/` built against it (one `ServicePage` layout component + per-page content modules in `lib/service-pages/`; pages exist exactly when registered, which is how the flagship gate is enforced in code). Three components from older specs are now BUILT as shared primitives, with these corrections recorded: **CtaBand** (shared/cta-band.md) ships WITHOUT the spec's GridLines and corner marks, retired with the instrument layer (the spec pre-dated D1); content centers at EDGE, the primary pill inverts via the accent scope, and the Bracket CTA is the band's annotation. **Faq** (6.7) renders the accordion and its FAQPage JSON-LD from one array; first instance carries D5's service-specific questions. **ProcessCard** (6.4) built as specced, no day-range chips on T2. New 6.15, the interior breadcrumb row (mono trail on a drawn hairline mirroring the BreadcrumbList JSON-LD's real URLs). Interior register confirmed in the build: zero pinned runways, no canvas, annotations 2 of 3 (H1 underline on the keyword, CTA bracket), theme rhythm light/tint alternation with no dark section until a sourced proof band exists, and the T2 proof slot renders NOTHING until real case studies land (the 6.8 no-placeholders-in-chrome rule applied to a section). Verified at 375/768/1280/1536 plus reduced motion on the live dev server: no overflow, reduced motion fully settled, one H1, Service + BreadcrumbList + FAQPage + Organization JSON-LD present, 850+ words server-rendered. GATE OPEN ITEM for Brad: the flagship review itself; flagged knob, the hero's right half at 1536 is deliberately empty (quiet interior posture), alternatives noted in the session log if he wants more furniture.
