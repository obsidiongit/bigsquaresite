...
Lead magnets (Brad 2026-08-30): the 5 titles on /resources/ are working titles only; the real list is still on Brad's whiteboard. 16 candidates + a suggested 5 in `project-sections/lead-magnets/brainstorm-2026-08-31.md`; Brad picks. When decided: 5 rows in `lib/resources.ts`, the 5 specs in `project-sections/lead-magnets/`, then the assets. Blog authors: the 6 team members are registered in `lib/blog-authors.ts` (2026-08-30; launch posts bylined Brad Brown + Mike Soden). Owed: headshots (Brad drops sources in `assets/team/`; export square crops to `public/media/` + map the 6 `blog-author-*` slots in `lib/asset-files.ts`). Covers: AI-generated figures PASSED Brad's first test 2026-08-30 (prompt + amended crop rule in `blog-plan.md` 2c). Next (no manual cropping, Brad's call): save raw generations to `assets/blog-covers/<slot-id>.png` and headshots to `assets/team/<first>.jpg`, then `npm run blog:assets` (scripts/blog-assets.mjs, built 2026-08-30) crops, converts, and wires everything; the developer weaves generation + this script into the writer job per 2c. Manual template spec stays in 2b as the fallback.
