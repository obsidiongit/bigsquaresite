...
Lead magnets (Brad 2026-08-30): the 5 titles on /resources/ are working titles only; the real list is still on Brad's whiteboard. 16 candidates + a suggested 5 in `project-sections/lead-magnets/brainstorm-2026-08-31.md`; Brad picks. When decided: 5 rows in `lib/resources.ts`, the 5 specs in `project-sections/lead-magnets/`, then the assets. Blog authors, decided 2026-08-30: team members take the byline with headshots, cycled per post. Owed: names + roles into `lib/blog-authors.ts` and one square headshot each (`public/media/` + `lib/asset-files.ts`). Cover template: Brad makes it himself; the exact spec (2400x1200, figure-not-poster, FIG stamp) is `project-sections/blog/blog-plan.md` section 2b.
